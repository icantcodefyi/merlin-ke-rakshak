...
const caseList = [
    { id: nanoid(), value: "True" },
    { id: nanoid(), value: "False" },
];
            ...
            <div className="flex flex-col divide-y divide-dark-200">
                <div className="relative min-h-10 flex flex-col">
                    <div className="flex flex-col p-4">
                        <div className="text-xs text-light-900/50 font-medium">Condition Description</div>

                        <div className="mt-2">
                            <textarea
                                value={data.userInput}
                                onChange={(e) => onUserInputChange(e.target.value)}
                                placeholder="Describe your condition here. Example: If the user's age is above 18..."
                                className="min-h-[80px] w-full resize-y border border-dark-50 rounded-md bg-dark-300 p-2 text-sm text-light-900 outline-none focus:border-purple-600"
                            />
                        </div>
                    </div>
                ...
                <div className="flex flex-col p-4">
                    <div className="text-xs text-light-900/50 font-medium">Paths</div>

                    {data.paths.length > 0 && (
                        <div className="mt-2 flex flex-col gap-2">
                            {data.paths.map((path) => (
                                <div key={path.id} className="relative">
                                    <NodePath id={path.id} path={path.case} onRemove={(_id) => removeNodePath(_id)} isConnectable={isConnectable} />
                                    <CustomHandle
                                        type="source"
                                        id={path.id}
                                        position={Position.Right}
                                        isConnectable={isConnectable}
                                        className="hover:(important:ring-2 important:ring-purple-500/50)"
                                    />
                                </div>
                            ))}
                        </div>
                ...
                <div className="px-4 py-2">
                    <div className="text-xs text-light-900/50">Describe your condition above. Add True and False paths to define the flow based on the condition result.</div>
                </div>
...
export const metadata: RegisterNodeMetadata<ConditionalPathNodeData> = {
    ...
    detail: {
        ...
        description: "Create conditional flows based on your custom conditions.",
    },
    connection: {
        inputs: 1,
        outputs: 2,
