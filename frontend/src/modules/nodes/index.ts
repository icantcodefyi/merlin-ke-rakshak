...
export const NODES: RegisterNodeMetadata[] = Object.values(import.meta.glob("~/modules/nodes/nodes/**/*.node.tsx", { eager: true }))
    .map((module: any) => module.metadata)
    .filter(Boolean);
...
export const NODES_METADATA = NODES.reduce(
    (acc, current) => {
